...
def create_or_update_list(list_name: str, items: List[Dict[str, Any]] = None) -> Dict[str, Any]:
...
def select_list_with_fzf(lists: List[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
    ...
    try:
        process = subprocess.run(
            ["fzf", "--height=40%", "--layout=reverse", "--prompt=Select list: "],
            input="\n".join(options),
            text=True,
            capture_output=True
        )
...
def select_item_with_fzf(items: List[Dict[str, Any]], prompt: str = "Select item: ") -> Optional[Dict[str, Any]]:
...
def search_lists_by_text(lists: List[Dict[str, Any]], search_text: str) -> List[Dict[str, Any]]:
...
def export_list_to_yaml(list_data: Dict[str, Any], output_file: Path):
    ...
    try:
        ...
        clean_data = {k: v for k, v in list_data.items() if k not in ['file_path', 'filename']}
...
def import_list_from_yaml(list_name: str, input_file: Path) -> bool:
    ...
    try:
        ...
        if isinstance(data, dict) and 'items' in data:
            ...
            typer.echo("Invalid YAML format: expected a dictionary with 'items' key", err=True)
            return False
    except Exception as e:
...
def create_list(
    ...
    # Create new list
    list_data = create_or_update_list(name)
...
def edit_list(
    ...
    item: Annotated[Optional[str], typer.Option("--item", help="Add an item to the list")] = None,
    remove_item: Annotated[Optional[str], typer.Option("--remove-item", help="Remove an item from the list")] = None,
    check: Annotated[Optional[str], typer.Option("--check", help="Mark an item as checked")] = None,
    uncheck: Annotated[Optional[str], typer.Option("--uncheck", help="Mark an item as unchecked")] = None,
):
    """Edit a list"""
    if not name:
        typer.echo("Error: List name is required", err=True)
    ...
    if not any([item, remove_item, check, uncheck]):
        typer.echo("No operation specified. Use --help to see available options.", err=True)
...
def delete_list(
    ...
    if not force:
        ...
        if not typer.confirm(f"Are you sure you want to delete list '{name}' with {item_count} items?"):
...
def search_lists(
    search: Annotated[str, typer.Option("--search", help="Search for lists or items by name or content")],
    no_interactive: Annotated[bool, typer.Option("--no-interactive", help="Disable interactive selection even if fzf is available")] = False,
):
    """Search lists"""
    if not search:
...
def export_list(
    ...
    export: Annotated[str, typer.Option("--export", help="Export the list to a file in CSV or YAML format")],
):
    ...
    if extension == '.csv':
        ...
        typer.echo("Error: Export format must be CSV (.csv) or YAML (.yaml/.yml)", err=True)
        raise typer.Exit(code=1)

def import_list(
    name: Annotated[str, typer.Option("--name", help="Name of the list to import to")],
    import_file: Annotated[str, typer.Option("--import", help="Import a list from a CSV or YAML file")],
):
    ...
    if extension == '.csv':
        ...
        typer.echo("Error: Import format must be CSV (.csv) or YAML (.yaml/.yml)", err=True)
        raise typer.Exit(code=1)
