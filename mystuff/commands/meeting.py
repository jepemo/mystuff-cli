...
def add_meeting(
    title: Annotated[
        str, typer.Option("--title", help="Title of the meeting (required)")
    ],
    date: Annotated[
        Optional[str],
        typer.Option(
            "--date",
            help="Date of the meeting in YYYY-MM-DD format (defaults to today)",
        ),
    ] = None,
    participants: Annotated[
        Optional[str],
        typer.Option("--participants", help="Comma-separated list of participants"),
    ] = None,
    body: Annotated[
        Optional[str],
        typer.Option("--body", help="Free-text content or agenda of the meeting"),
    ] = None,
    template: Annotated[
        Optional[str],
        typer.Option(
            "--template",
            help="Path to a template file for pre-filling the meeting body",
        ),
    ] = None,
    tags: Annotated[
        Optional[List[str]],
    ...
    no_edit: Annotated[
        bool,
        typer.Option(
            "--no-edit", help="Don't prompt to edit the meeting note after creation"
        ),
    ] = False,
):
...
def list_meetings(
    search: Annotated[
        Optional[str],
        typer.Option("--search", help="Search by title, date, or participants"),
    ] = None,
    tag: Annotated[
    ...
    date_filter: Annotated[
        Optional[str],
        typer.Option("--date", help="Filter by specific date (YYYY-MM-DD)"),
    ] = None,
    interactive: Annotated[
        bool,
        typer.Option(
            "--interactive", "-i", help="Use fzf for interactive selection and preview"
        ),
    ] = False,
):
    ...
    if not meeting_files:
        typer.echo(
            "No meeting notes found. Use 'mystuff meeting add --title <TITLE>' "
            "to create your first meeting note."
        )
        return
    ...
    if search:
        ...
        meetings = [
            ...
            if (
                ...
                or search_lower in m.get("date", "").lower()
                or any(search_lower in p.lower() for p in m.get("participants", []))
            )
        ]
    ...
    if interactive and is_fzf_available():
        typer.echo(
            "🔍 Interactive meeting browser (press Enter to view, Ctrl+C to exit):"
        )
        ...
        if selected_meeting:
            ...
            if selected_meeting.get("participants"):
                typer.echo(
                    f"   Participants: {', '.join(selected_meeting['participants'])}"
                )
...
def edit_meeting(
    title: Annotated[
        Optional[str],
        typer.Option(
            "--title", help="Title of the meeting to edit (optional if using fzf)"
        ),
    ] = None,
    date: Annotated[
        Optional[str],
        typer.Option("--date", help="Date of the meeting (if title is not unique)"),
    ] = None,
):
    ...
    if not selected_meeting:
        if not title:
            typer.echo(
                "Error: Title is required to identify the meeting to edit "
                "(or install fzf for interactive selection)",
                err=True,
            )
            raise typer.Exit(code=1)
...
def delete_meeting(
    title: Annotated[
        Optional[str],
        typer.Option(
            "--title", help="Title of the meeting to delete (optional if using fzf)"
        ),
    ] = None,
    date: Annotated[
        Optional[str],
    ...
    if not selected_meeting:
        if not title:
            typer.echo(
                "Error: Title is required to identify the meeting to delete "
                "(or install fzf for interactive selection)",
                err=True,
            )
            raise typer.Exit(code=1)
    ...
    if typer.confirm(
        f"Are you sure you want to delete '{selected_meeting['title']}' "
        f"({selected_meeting['date']})?"
    ):
        file_path.unlink()
...
def search_meetings(
    ...
    for file_path in meeting_files:
        try:
            meeting = load_meeting_from_file(file_path)
            if (
                query_lower in meeting.get("title", "").lower()
                or query_lower in meeting.get("date", "").lower()
                or any(
                    query_lower in p.lower() for p in meeting.get("participants", [])
                )
                or query_lower in meeting.get("body", "").lower()
            ):
                matching_meetings.append(meeting)
        except Exception:
            continue
