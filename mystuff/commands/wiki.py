...
def save_wiki_to_file(file_path: Path, title: str, tags: List[str], aliases: List[str], backlinks: List[str], body: str):
...
def select_wiki_with_fzf(notes: List[dict]) -> Optional[dict]:
    ...
    for note in notes:
        ...
        aliases_str = f"({', '.join(note.get('aliases', []))})" if note.get('aliases') else ""
...
def new_wiki_note(
    ...
    aliases: Annotated[Optional[List[str]], typer.Option("--alias", help="One or more aliases for the note")] = None,
    body: Annotated[Optional[str], typer.Option("--body", help="Content for the wiki note")] = None,
    no_edit: Annotated[bool, typer.Option("--no-edit", help="Don't open editor after creation")] = False,
):
...
def view_wiki_note(
    title: Annotated[str, typer.Argument(help="Title or alias of the wiki note to view")],
    graph: Annotated[bool, typer.Option("--graph", help="Display ASCII graph of backlinks")] = False,
):
    """View a wiki note"""
...
def edit_wiki_note(
    title: Annotated[str, typer.Argument(help="Title or alias of the wiki note to edit")],
):
...
def delete_wiki_note(
    title: Annotated[str, typer.Argument(help="Title or alias of the wiki note to delete")],
...
def list_wiki_notes(
    ...
    if not no_interactive and is_fzf_available():
        ...
        if selected_note:
            ...
            if open_editor(file_path):
                typer.echo(f"✅ Wiki note opened: {selected_note.get('title', 'Untitled')}")
                # Update backlinks after editing
                update_backlinks()
        return
...
def search_wiki_notes(
    ...
    graph: Annotated[bool, typer.Option("--graph", help="Display ASCII graph of backlinks for results")] = False,
    no_interactive: Annotated[bool, typer.Option("--no-interactive", help="Disable interactive selection even if fzf is available")] = False,
):
