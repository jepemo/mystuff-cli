...
def filter_evaluations_by_date_range(
    evaluations: List[dict], start_date: str, end_date: str
) -> List[dict]:
...
def generate_report(evaluations: List[dict]) -> str:
    ...
    # Recent entries
    recent_evals = sorted(evaluations, key=lambda x: x.get("date", ""), reverse=True)[
        :10
    ]
...
def add_evaluation(
    category: Annotated[
        ...
        typer.Option(
            "--category",
            help="Category of the evaluation (e.g., 'productivity', 'health')",
        ),
    ],
    score: Annotated[
        int, typer.Option("--score", help="Numeric score for the evaluation (1-10)")
    ],
    date: Annotated[
        Optional[str],
        typer.Option(
            "--date",
            help="Date of the evaluation in YYYY-MM-DD format (defaults to today)",
        ),
    ] = None,
    comments: Annotated[
        Optional[str],
        typer.Option(
            "--comments", help="Optional free-text comments for the evaluation"
        ),
    ] = None,
):
...
def list_evaluations(
    limit: Annotated[
        Optional[int],
        typer.Option(
            "--limit", "-l", help="Limit the number of evaluations to display"
        ),
    ] = None,
    category: Annotated[
    ...
    if category:
        evaluations = [
            e for e in evaluations if e.get("category", "").lower() == category.lower()
        ]

    # Filter by date range if specified
    if date_range:
        start_date, end_date = parse_date_range(date_range)
        evaluations = filter_evaluations_by_date_range(
            evaluations, start_date, end_date
        )
...
def edit_evaluation(
    category: Annotated[
        str, typer.Option("--category", help="Category of the evaluation to edit")
    ],
    date: Annotated[
        Optional[str],
        typer.Option(
            "--date", help="Date of the evaluation to edit (defaults to today)"
        ),
    ] = None,
    score: Annotated[
        Optional[int],
        typer.Option("--score", help="New score for the evaluation (1-10)"),
    ] = None,
    comments: Annotated[
        Optional[str],
        typer.Option("--comments", help="New comments for the evaluation"),
    ] = None,
):
    ...
    new_comments = (
        comments if comments is not None else existing_eval.get("comments", "")
    )
...
def delete_evaluation(
    category: Annotated[
        str, typer.Option("--category", help="Category of the evaluation to delete")
    ],
    date: Annotated[
        Optional[str],
        typer.Option(
            "--date", help="Date of the evaluation to delete (defaults to today)"
        ),
    ] = None,
    force: Annotated[
    ...
    if not force:
        ...
        if not typer.confirm(
            f"Are you sure you want to delete evaluation '{category}' "
            f"on {date} (score: {score})?"
        ):
...
def generate_evaluation_report(
    date_range: Annotated[
        Optional[str],
        typer.Option(
            "--range", help="Date range for the report (YYYY-MM-DD:YYYY-MM-DD)"
        ),
    ] = None,
    category: Annotated[
        Optional[str], typer.Option("--category", help="Filter report by category")
    ] = None,
):
    """Generate a summary report of evaluations"""
    evaluations = get_all_evaluations()
