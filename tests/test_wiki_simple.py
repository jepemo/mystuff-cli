...
def test_wiki_new_basic():
    ...
    from mystuff.commands.wiki import (
        get_wiki_filename,
        load_wiki_from_file,
        save_wiki_to_file,
    )
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        assert (
            filename == "test-wiki-note.md"
        ), f"Expected test-wiki-note.md, got {filename}"
        ...
        # Verify content
        assert (
            loaded_note["title"] == title
        ), f"Expected title {title}, got {loaded_note['title']}"
        assert (
            loaded_note["tags"] == tags
        ), f"Expected tags {tags}, got {loaded_note['tags']}"
        assert (
            loaded_note["aliases"] == aliases
        ), f"Expected aliases {aliases}, got {loaded_note['aliases']}"
        assert (
            loaded_note["backlinks"] == backlinks
        ), f"Expected backlinks {backlinks}, got {loaded_note['backlinks']}"
        assert (
            loaded_note["body"] == body
        ), f"Expected body {body}, got {loaded_note['body']}"
...
def test_wiki_slugify():
    ...
    from mystuff.commands.wiki import get_wiki_filename, slugify
    ...
    for title, expected_slug in test_cases:
        ...
        ), f"Expected {expected_slug}, got {result} for title '{title}'"

        # Test filename generation
        filename = get_wiki_filename(title)
        assert (
            filename == f"{expected_slug}.md"
        ), f"Expected {expected_slug}.md, got {filename}"
...
def test_wiki_load_without_frontmatter():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # Verify content
        assert (
            loaded_note["title"] == "Test Note"
        ), f"Expected title 'Test Note', got {loaded_note['title']}"
        assert (
            loaded_note["tags"] == []
        ), f"Expected empty tags, got {loaded_note['tags']}"
        assert (
            loaded_note["aliases"] == []
        ), f"Expected empty aliases, got {loaded_note['aliases']}"
        assert (
            loaded_note["backlinks"] == []
        ), f"Expected empty backlinks, got {loaded_note['backlinks']}"
        assert (
            loaded_note["body"] == body
        ), f"Expected body {body}, got {loaded_note['body']}"
...
def test_wiki_get_all_notes():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        ), f"Expected titles {expected_titles}, got {titles}"
...
def test_wiki_find_by_title_and_alias():
    ...
    from mystuff.commands.wiki import (
        find_wiki_note_by_title_or_alias,
        get_wiki_filename,
        save_wiki_to_file,
    )
...
def test_wiki_search():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        assert (
            results[0]["title"] == "Python Programming"
        ), f"Expected 'Python Programming', got {results[0]['title']}"

        # Search by tag
        results = search_notes_by_text(notes, "programming")
        assert (
            len(results) == 2
        ), f"Expected 2 results for 'programming', got {len(results)}"
        ...
        assert (
            results[0]["title"] == "JavaScript Guide"
        ), f"Expected 'JavaScript Guide', got {results[0]['title']}"

        # Search by body content
        results = search_notes_by_text(notes, "quarter")
        assert len(results) == 1, f"Expected 1 result for 'quarter', got {len(results)}"
        assert (
            results[0]["title"] == "Project Planning"
        ), f"Expected 'Project Planning', got {results[0]['title']}"
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        notes_data = [
            (
                ...
                [],
                [],
                "This project involves [[Team Structure]] and [[Development Process]].",
            ),
            (
                "Team Structure",
                [],
                [],
                "The team works on [[Project Overview]] using [[Development Process]].",
            ),
            (
                "Development Process",
                [],
                [],
                "Our process supports the [[Project Overview]].",
            ),
        ...
        assert (
            actual_backlinks == expected_backlinks
        ), f"Expected backlinks {expected_backlinks}, got {actual_backlinks}"
        ...
        assert (
            standalone_note.get("backlinks", []) == []
        ), "Standalone note should have no backlinks"
