...
from mystuff.commands.sync import sync_app, get_mystuff_dir, load_sync_config, execute_sync_commands
...
def test_sync_run_command_no_mystuff_dir(runner):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            ...
            assert "No mystuff directory found" in result.stdout
        finally:
            os.chdir(original_cwd)
...
def test_sync_with_invalid_config(runner):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            ...
            result = runner.invoke(sync_app, ["run"])
            assert result.exit_code == 1
            assert "Error parsing config.yaml" in result.stdout
        finally:
            os.chdir(original_cwd)
