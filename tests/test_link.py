...
def test_link_add_auto_title():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        result = runner.invoke(
            app,
            ["link", "add", "--url", "https://github.com/example/repo"],
            env={"MYSTUFF_HOME": str(test_dir)},
        )
...
def test_link_add_duplicate():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        result = runner.invoke(
            app,
            ["link", "add", "--url", "https://example.com", "--title", "Example Site"],
            env={"MYSTUFF_HOME": str(test_dir)},
        )
...
def test_link_list_empty():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # List links
        result = runner.invoke(
            app, ["link", "list"], env={"MYSTUFF_HOME": str(test_dir)}
        )
...
def test_link_search():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # Search by title
        result = runner.invoke(
            app, ["link", "search", "python"], env={"MYSTUFF_HOME": str(test_dir)}
        )
        ...
        result = runner.invoke(
            app, ["link", "search", "repository"], env={"MYSTUFF_HOME": str(test_dir)}
        )
...
def test_link_delete():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        result = runner.invoke(
            app,
            ["link", "add", "--url", "https://github.com", "--title", "GitHub"],
            env={"MYSTUFF_HOME": str(test_dir)},
        )
        assert result.exit_code == 0
        ...
        result = runner.invoke(
            app,
            ["link", "delete", "--url", "https://example.com"],
            env={"MYSTUFF_HOME": str(test_dir)},
        )
...
def test_link_delete_nonexistent():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        result = runner.invoke(
            app,
            ["link", "delete", "--url", "https://nonexistent.com"],
            env={"MYSTUFF_HOME": str(test_dir)},
        )
