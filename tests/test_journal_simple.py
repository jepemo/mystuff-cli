...
def test_journal_add_basic():
    ...
    from mystuff.commands.journal import save_journal_to_file, load_journal_from_file
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # Verify content
        assert loaded_entry['date'] == date, f"Expected date {date}, got {loaded_entry['date']}"
        assert loaded_entry['tags'] == tags, f"Expected tags {tags}, got {loaded_entry['tags']}"
        assert loaded_entry['body'] == body, f"Expected body {body}, got {loaded_entry['body']}"
        assert loaded_entry['file_path'] == file_path, f"Expected file_path {file_path}, got {loaded_entry['file_path']}"
...
def test_journal_load_without_frontmatter():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # Verify content
        assert loaded_entry['date'] == "2023-10-05", f"Expected date 2023-10-05, got {loaded_entry['date']}"
        assert loaded_entry['tags'] == [], f"Expected empty tags, got {loaded_entry['tags']}"
        assert loaded_entry['body'] == body, f"Expected body {body}, got {loaded_entry['body']}"
        assert loaded_entry['file_path'] == file_path, f"Expected file_path {file_path}, got {loaded_entry['file_path']}"
...
def test_journal_search():
    ...
    from mystuff.commands.journal import search_entries_by_text, save_journal_to_file
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # Create test entries
        entries = [
            {'date': '2023-10-01', 'body': 'Today I learned Python programming', 'tags': ['learning']},
            {'date': '2023-10-02', 'body': 'Had a great meeting with the team', 'tags': ['work']},
            {'date': '2023-10-03', 'body': 'Spent time reading about JavaScript', 'tags': ['learning', 'javascript']},
            {'date': '2023-10-04', 'body': 'Relaxing day with family', 'tags': ['personal']},
        ]
        
        # Search by body content
        results = search_entries_by_text(entries, 'Python')
        assert len(results) == 1, f"Expected 1 result for 'Python', got {len(results)}"
        assert results[0]['date'] == '2023-10-01', f"Expected date 2023-10-01, got {results[0]['date']}"
        
        # Search by tag
        results = search_entries_by_text(entries, 'learning')
        assert len(results) == 2, f"Expected 2 results for 'learning', got {len(results)}"
        ...
        assert results[0]['date'] == '2023-10-02', f"Expected date 2023-10-02, got {results[0]['date']}"
...
def test_journal_date_range():
    ...
    from mystuff.commands.journal import parse_date_range, filter_entries_by_date_range
